...
def rainbowCycle(wait_ms=50, cycles=1):
    """
    Create a rainbow effect by cycling through colors on both LEDs
    wait_ms: delay between color changes in milliseconds
    cycles: number of complete rainbow cycles
    """
    # Rainbow color sequence (R, G, B values)
    rainbow_colors = [
        (255, 0, 0),    # Red
        (255, 127, 0),  # Orange
        (255, 255, 0),  # Yellow
    ...
    for cycle in range(cycles):
        for color in rainbow_colors:
            r, g, b = color
            ledIndex(0, r, g, b)  # Set LED 0
            ledIndex(1, r, g, b)  # Set LED 1
            time.sleep(wait_ms / 1000.0)  # Convert ms to seconds

def theaterChaseRainbow(wait_ms=50):
    """
    Create a theater chase effect with rainbow colors
    Alternates LEDs with different colors for chase effect
    """
    chase_colors = [
        (255, 0, 0),    # Red
        (0, 255, 0),    # Green  
    ...
    for color in chase_colors:
        r, g, b = color
        # LED 0 gets the color, LED 1 gets opposite/complementary
        ledIndex(0, r, g, b)
        ledIndex(1, 255-r, 255-g, 255-b)  # Complementary color
...
def robot_dance():
    ...
    for i in range(3):
        rainbowCycle(100, 1)  # Rainbow cycle effect
        time.sleep(0.5)
    ...
    for rep in range(6):  # 6 reps for full rotation effect
        turnRight()
        time.sleep(0.3)
        theaterChaseRainbow(50)  # Chase lights during spin
    ...
    for rep in range(6):  # 6 reps for full counter-rotation
        turnLeft()
        time.sleep(0.3)
        ledIndex(1, 255, 0, 255)  # Magenta LEDs
    ...
    ledIndex(0, 255, 255, 255)  # White LED
    ledIndex(1, 255, 255, 255)  # White LED
    ...
    for rep in range(8):  # 8 spins for full rotation
        ...
        if rep % 4 == 0:
            ...
            ledIndex(0, 255, 0, 0)    # Red
            ledIndex(1, 255, 0, 0)
        else:
            ledIndex(0, 0, 255, 0)    # Green
            ledIndex(1, 0, 255, 0)
